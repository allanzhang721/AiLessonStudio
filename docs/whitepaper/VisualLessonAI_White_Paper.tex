...
The repository test discovery contains 39 tests at this paper version. It covers frame scoring and short-circuit behavior, explanation repair, provider routing, the conditional second-key Streamlit flow, concept relationship direction, progress callbacks, waiting-tip selection, source annotation conversion, schema normalization, Markdown and LaTeX handling, fail-fast plan validation, student-analysis helpers, narration arguments, audio-aware timing, and real H.264 encoding. The command used for the deployment branch is:
...
\begin{lstlisting}[language=bash]
cd docs/whitepaper
pdflatex VisualLessonAI_White_Paper.tex
bibtex VisualLessonAI_White_Paper
pdflatex VisualLessonAI_White_Paper.tex
pdflatex VisualLessonAI_White_Paper.tex
\end{lstlisting}
