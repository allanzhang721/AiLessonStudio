...
\author{%
\begin{tabular}{ccc}
\textbf{Meiyu Shen} & \textbf{Yifan Yan} & \textbf{Jiachen Yu}
\end{tabular}\\[0.55em]
\normalsize Jiaxing Senior High School\\[-0.05em]
\small BC Offshore Program\\[0.35em]
\footnotesize\itshape All authors contributed equally to this work.
}
\date{August 2026}
...
\begin{tikzpicture}[
 ...
 box/.style={draw=navy,rounded corners=3pt,fill=pale,minimum height=11mm,text width=26mm,align=center,font=\footnotesize},
 gate/.style={diamond,draw=amber!85!black,fill=amber!12,aspect=2.0,align=center,font=\footnotesize},
 callout/.style={fill=white,inner sep=1.5pt,text=red!75!black,text width=22mm,align=center,font=\scriptsize},
 arrow/.style={-{Latex[length=2mm]},thick,draw=navy},
 reject/.style={-{Latex[length=2mm]},draw=red!75!black,dashed}
]
...
\node[box,below=18mm of evidence] (plan) {Seven-step\\visual plan};
...
\node[callout,above=6mm of g1] (g1fail) {Block or\\one repair};
\node[callout,below=6mm of g2] (g2fail) {Needs review;\\no final MP4};
\draw[reject] (g1.north) -- (g1fail.south);
\draw[reject] (g2.south) -- (g2fail.north);
\end{tikzpicture}
...
The project retains comparison results from DistilBERT, BERT, and RoBERTa experiments. BERT introduced bidirectional transformer pretraining, RoBERTa revised its training recipe, and DistilBERT compressed a larger teacher model \citep{devlin2019bert,liu2019roberta,sanh2019distilbert}. Those experiments classify pedagogical error categories on a fixed dataset. The compact results are useful offline evidence, but the training checkpoints and optimizer state are not part of the production distribution, and the task is not the same as verifying an arbitrary new explanation. We therefore report the results as baselines and use the narrower, inspectable cascade in production.
...
\node[
  ...
  align=center
] at (3.43,-0.38) {Shared scene bible: stable objects and consistent color meanings};
\end{tikzpicture}
...
The project includes saved benchmark summaries for the earlier research checkers. Table~\ref{tab:benchmarks} reports the values used by the application's friendly experiment snapshot. Gate 1 and Gate 2 use different datasets and tasks, so their rows must not be ranked against one another.
...
\end{subfigure}
\caption{Offline experiment figures generated from saved experiment records. These plots do not show live request latency or learning outcomes.}
...
\section{Runtime Requirements and Test Protocol}

The production target is Python 3.12. The minimal application dependencies cover model-provider access, image processing, video encoding, web requests, the Streamlit interface, and graph rendering. Research and training dependencies are isolated from the public application so they do not increase website startup time.
...
The automated suite contains 33 production tests at this paper version. It covers frame scoring and short-circuit behavior, explanation repair, provider routing, the conditional second-key flow, concept relationship direction, progress callbacks, waiting-tip selection, source annotation conversion, schema normalization, Markdown and LaTeX handling, fail-fast plan validation, student-analysis helpers, narration arguments, audio-aware timing, and real H.264 encoding.

A passing unit suite establishes that the implementation meets the tested contracts. It does not establish educational effectiveness. A publication evaluation should additionally include expert factual review, student comprehension measures, accessibility review, cost records, and failure analysis.
...
\node[box,left=of state] (source) {Deployment source\\code, no visitor keys};
...
\draw[arr] (source)--node[above,font=\scriptsize]{deploy}(streamlit);
...
The public deployment uses Streamlit Community Cloud with Python 3.12. The hosting service installs declared dependencies and supports configuration of runtime settings and secrets \citep{streamlitDeploy,streamlitSecrets}. The current visitor flow does not require a shared provider secret because the visitor provides a session text key and, only when pairing a text-only provider with media, an optional session image key.
...
\midrule
Figures~\ref{fig:architecture}--\ref{fig:deployment} & Drawn in TikZ from the production implementation and deployment configuration & Explanatory system diagrams; not experimental observations \\
Figure~\ref{fig:offlineplots} & Saved plotting outputs and benchmark values & Offline method comparison on recorded tasks \\
Table~\ref{tab:benchmarks} & Saved production benchmark summary & Distilled values used by the public dashboard \\
Gate, timing, progress, and graph equations & Production gate, media, and lesson logic & Exact implementation or direct algebraic consequence as of the paper version \\
...
Setup instructions & Project configuration and official Streamlit documentation & Reproducibility and deployment procedure \\
\bottomrule
\end{tabularx}
...
\begin{enumerate}
  \item Obtain administrator access to the chosen hosting project.
  \item Confirm that the Python runtime and production dependencies are declared.
  \item Run the production test protocol on Python 3.12.
  \item Keep provider credentials outside the published source and configuration examples.
  \item Deploy the application, inspect build logs, open the public URL, and confirm that visibility settings match the intended audience.
  \item Test both provider paths: OpenAI should expose text, research, image, and voice controls; DeepSeek should show a text-only notice and reveal the optional OpenAI media-key path only after the learner enables it.
  \item Generate a text-only lesson, an illustrated lesson, a failed-key request, a Gate 1 repair, and a visual regeneration.
  \item Record the release date, provider models, calibrated thresholds, and relevant environment details with published results.
\end{enumerate}

Official Streamlit guidance recommends declaring dependencies, selecting the intended Python version, and keeping secrets outside published source material \citep{streamlitDeploy,streamlitSecrets}. Access to the hosting project is required to change deployment visibility and runtime configuration.

\section{Version and Disclosure Statement}

This white paper describes the August 2026 production release. A publication package should archive the final PDF together with its bibliography, figure assets, software version, and a short change log, without embedding account-specific hosting locations in the paper.

Generative AI assisted with organization, editing, and diagram planning. The authors remain responsible for checking the implementation claims, citations, equations, and publication permissions. The uploaded SLaTR manuscript was consulted only for general writing habits and exposition structure. It is marked as an anonymous submission whose sharing is prohibited, so it is neither cited as technical evidence nor reproduced.
